						...
						<section id="flip-scroll">
							<table class="display table table-bordered cf" id="dyntable">
								...
								<tbody>
									...
									$query = mysql_query("SELECT * FROM sales_old WHERE `cat`='$dept1' GROUP BY saleorder_id");
									...
									<tr>
										...
										<?php $sale_total = mysql_fetch_array(mysql_query("SELECT cat,sum(sale_total) as approve_total,saleorder_id,userid ,sum(sale_qty) as approve_qty,sum(total) as order_total,sum(qty) as order_qty FROM sales_old WHERE `cat`='$dept1' AND `saleorder_id`='$salesid'"));
										...
										<!--onclick="printDiv(<?php  echo $salesid;?>,<?php  echo $dept1;?>,'<?php echo $userid; ?>')"-->
										<a onclick="multiple_print(<?php  echo $salesid;?>,<?php  echo $dept1;?>,'<?php echo $userid; ?>')" class="block_tip tooltips"><i class="fa fa-print"></i></a>
										</td>
									</tr>
									<?php $i++; } ?>
								</tbody>
