<?php  include('../admin/connect/config.php');error_reporting(1);extract($_POST); 
     if($appstatus == 0)
	 {
	$ca=mysql_query("UPDATE `sub_category` SET `appstatus`=1  WHERE  `dept`=0 AND `id`='$subid' ");
	 ...
	 {
		 $ca=mysql_query("UPDATE `sub_category` SET `appstatus`=0  WHERE  `dept`=0 AND `id`='$subid' ");
