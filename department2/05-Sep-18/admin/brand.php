...
if($num_brand < 1)
{
mysql_query("INSERT INTO `brand` (`dept`,`category`,`sub_categoryname`,`brandname`,`date`,`dtime`) VALUES ('0','$category','$sub_categoryname','$brandname','".date('Y-m-d')."','".date('Y-m-d H:i:s')."')");
...
<?php $ca=mysql_query("SELECT * FROM `department` WHERE `status`=1");
while($cat=mysql_fetch_array($ca)){ ?>
<option value="<?php echo $cat['id'].'@'.$cat['department'];?>" <?php if($cat['id']==$category){echo "selected";}?>><?php echo $cat['department'];?></option>
<?php } ?>
</select>
					</div>
				...
				<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Sub Category <span class="red">*</span></label>
						<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
						<select name="sub_categoryname" id="sub_categoryname" class="form-control selectpicker" required  data-show-subtext="true" data-live-search="true" >
						<option>---Select Brand---</option>
						<?php $ca=mysql_query("SELECT * FROM `sub_category` WHERE  `dept`=0 AND `status`=1");
