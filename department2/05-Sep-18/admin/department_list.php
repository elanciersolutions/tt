...
extract($_GET);
if($cmd=='act'){
mysql_query("UPDATE `department` SET `status`=1 WHERE `id`='$id'");
} else if($cmd=='deac') {
mysql_query("UPDATE `department` SET `status`=0 WHERE `id`='$id'");
}
			...
			<div class="panel-body">
				<section id="flip-scroll">
					<table class="display table table-bordered cf" id="dyntable">
						...
						<tbody>
							<?php
							$query=mysql_query("SELECT * FROM `department` ORDER BY `id` DESC");
							$i=1; while($result=mysql_fetch_array($query)) { ?>		
							<tr>
								<td class="text-center"><?php echo $i;?></td>
								<td><?php echo $result['department'];?></td>
								<td><img width="50" height="50" src="../images/department/<?php echo $result['image'];?>"></td>
