...
$in = mysql_query("INSERT INTO `sub_category` (`dept`,`category`,`sub_categoryname`,`date`,`dtime`,`catid`) VALUES ('0','$category[1]','$sub_categoryname','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$category[0]')");
if($in)
{
...
extract($_POST);
$category = explode("@",$category);
$in=mysql_query("UPDATE `sub_category` SET `category`='$category[1]',`sub_categoryname`='$sub_categoryname',`catid`='$category[0]' WHERE `id`='$id'");
if($in){
echo "<script type='text/javascript'>swal({
...
<select name="category" id="category" class="form-control" required >
	...
	<option value="<?php echo $cat['id'].'@'.$cat['department'];?>" <?php if($cat['id']==$catid){echo "selected";}?>><?php echo $cat['department'];?></option>
	<?php } ?>
</select>
