...
if($num_brand < 1)
{
mysql_query("INSERT INTO `brand` (`dept`,`category`,`sub_categoryname`,`brandname`,`date`,`dtime`) VALUES ('$dept','$category','$sub_categoryname','$brandname','".date('Y-m-d')."','".date('Y-m-d H:i:s')."')");
...
<div class="panel-body">
	...
	<form class="cmxform form-horizontal tasi-form" id="" method="post" name="product" enctype="multipart/form-data" >
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Sub Category <span class="red">*</span></label>
				...
				<?php $ca=mysql_query("SELECT * FROM `sub_category` WHERE  `dept`='$dept' OR `dept`=0 AND `status`=1");
