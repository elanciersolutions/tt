...
<header class="panel-heading">
	...
	<div class="xtra_btn_top">
		<a title="Sale Order ID"  data-placement="top" data-toggle="tooltip" class="btn btn-success tooltips" >Sale Order ID : <?php 
		$salesid = $_REQUEST['salesid'];
		$dept1 = sessionvalue('department', $dept);
		
		$query_salesid = mysql_fetch_array(mysql_query("SELECT * FROM sales WHERE `dept`='$dept1' AND `id`='$salesid'"));
		
		echo $query_salesid['salesid'];
		...
		<table class="display table table-bordered cf" id="dyntable">
			...
			<tbody>
				...
				<tr>
					...
					echo $qty = $result['qty'];}else{echo $qty = $result['sale_qty'];} ?>" id="edit_qty<?php echo $tableid=$result['id'];?>" style="width:50px;text-align:center;"readonly onkeyup="edit_salesorder_new(<?php echo $salesid;?>,<?php echo $price;?>,<?php echo $prodid;?>,<?php echo $tableid=$result['id'];?>,this.value,<?php echo $result['qty'];?>,<?php
...
<footer class="panel-footer">
	...
	<?php   
	 $query1 = mysql_query("SELECT * FROM sales_details WHERE `dept`='$dept1' AND `sid`='$salesid'");
	$result1 = mysql_fetch_array($query1);
	if($result1['approve_status'] == 0)
	{
		?>
		<a title="Approve" data-placement="top" data-toggle="tooltip" class="btn btn-success tooltips" onclick="approve_status_new(<?php echo $salesid; ?>,<?php echo $dept1; ?>)">Approve</a>
  <?php 
	}
	else
	{
