...
<section class="panel">
	<header class="panel-heading">
		...
		<div class="xtra_btn_top">
			<a title="New" href="user.php" data-placement="top" data-toggle="tooltip" class="btn btn-success tooltips" >Add New</a>
		</div>
	</header>
		...
		<section id="flip-scroll">
			<table class="display table table-bordered cf" id="dyntable">
				...
				<tbody>
					...
					$query=mysql_query("SELECT * FROM `user` WHERE `dept`='$dept1' ORDER BY `id` DESC");
					$i=1; while($result=mysql_fetch_array($query)) { ?>		
					<tr>
						<td class="text-center"><?php echo $i;?></td>
						<td class="text-center"><?php echo in_dept('department', $result['department']);?></td>
						...
						<a class="block_tip tooltips" href="user_list.php?id=<?php echo $result['id'];?>&cmd=deac"><i class="fa fa-thumbs-up"></i></a>
						<?php } else if($result['status']==0){?>
						<a class="block_tip tooltips" href="user_list.php?id=<?php echo $result['id'];?>&cmd=act"><i class="fa fa-thumbs-down"></i></a>
						<?php } ?></td>
						<td class="text-center"><a href="user.php?id=<?php echo $result['id'];?>" class="block_tip tooltips"><i class="fa fa-pencil"></i></a></td>
					</tr>
					<?php $i++; } ?>
				</tbody>
			</table>
