	 ...
	 while($fetch_approve = mysql_fetch_array($select_approve))
	 {
	   ...
	   if($approve_sale)
	   {
	    ...
	    while($fetch_sale = mysql_fetch_array($sales_approve))
		{
		  ...
		  $approve_sales =mysql_query("UPDATE `sales` SET `approve_status`=1,`cscid`='$cscid',`sale_qty`='$approve_sale_qty',`sale_total`='$approve_sale_total'  WHERE  `dept`='$dept' AND `id`='$saleorder_id' ");
