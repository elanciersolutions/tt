...
$in=mysql_query("UPDATE `sponser` SET `incharge_name`='$incharge_name',`pancard`='$pancard',`adhar`='$adhar',`phone`='$phone',`sponsername`='$sponsername',`sponserid`='$sponserid',`sp_password`='$sp_password',assign_pincode='$pin2' WHERE `id`='$id'");
if($in){
echo "<script type='text/javascript'>swal({
...
?></div>
<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12" style="height:auto;" id="hubpin1"  >
<?php 

       if($id != "")
	   {
		   
		   $pincode_id = explode(",",$assign_pincode);
		   
		   foreach($pincode_id as $pinid)
		   {
			   $fetch_hub = mysql_fetch_array(mysql_query("SELECT * FROM `cse_count` WHERE `id`='$pinid'"));
			  
				 $pincode = $fetch_hub['pincode'];
				...
				&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" checked name="pin[]" id="pin" value="<?php echo $fetch_hub['id'];?>">&nbsp;&nbsp;&nbsp;<?php echo $pincode;?><br>
		    ...
		    foreach($pincode_id as $pinid)
		   {  
			$pincode1 = $fetch_hub1['pincode'];
?>		   
		   <!--<input type="hidden" name="hub[]" id="hub" value="<?php  echo  $hubid1 = $fetch_hub1['id']; ?>">&nbsp;&nbsp;&nbsp;<span style="font-weight:bold;font-size:16px;padding-bottom:100px;"><?php  echo  $hubname1 = $fetch_hub1['hubname']; ?></span> <br>
				&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="pin[]" id="pin" value="<?php echo $fetch_hub1['id'];?>">&nbsp;&nbsp;&nbsp;<?php echo $pincode1;?><br>-->
