				...
				<table class="display table table-bordered cf" id="dyntable">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center"><?php echo $result['incharge_name'];?></td>
							<td><?php echo $result['assign_city'];?></td>
							<td class="text-center"><?php  $pincode = explode(",",$result['assign_pincode']);
					foreach($pincode as $pin_id)
					{
			
			$fetch=mysql_fetch_array(mysql_query("SELECT * FROM `cse_count` WHERE `id`='$pin_id'"));
						
		echo "<b>".$fetch['hubname'].'</b>'.'-'.$fetch['pincode'].'</br></br>';
