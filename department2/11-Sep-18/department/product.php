...
$num_product = mysql_num_rows(mysql_query("SELECT * FROM product WHERE pname='$product_name' AND `subcategory`='$sub_categoryname' AND `brandname`='$brandname'"));
if($num_product < 1)
{
mysql_query("INSERT INTO `product` (`dept`,`category`,`subcategory`,`pname`,`date`,`dtime`,`commission`,`gst`,`image`,`price`,`brandname`,`featured`) VALUES ('$dept','$category','$sub_categoryname','$product_name','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$commission','$gst','$img','$price','$brandname','$featured')");
...
$in=mysql_query("UPDATE `product` SET `category`='$category',`subcategory`='$sub_categoryname',`pname`='$product_name',`commission`='$commission',`gst`='$gst',image='$img',`price`='$price',`brandname`='$brandname',`featured`='$featured' WHERE `id`='$id'");
if($in){
echo "<script type='text/javascript'>swal({
...
<div class="panel-body">
	...
	<form class="cmxform form-horizontal tasi-form" id="" method="post" name="product" enctype="multipart/form-data" >
		<div class="form-group">
			<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="department">Category </label>
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Sub Category <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
				...
				<select required name="sub_categoryname" id="sub_categoryname" class="form-control selectpicker" data-show-subtext="true" data-live-search="true" onchange="brand_fetch(this.value,<?php echo $dept1;?>,category.value )" >
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Brand <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12" id="brandname1">
			
				   
				<select required name="brandname" id="brandname" class="form-control" >
				<option value="">--- Select ---</option>
				<?php	 
				if($brandname != "")
				{
				
				 $ca=mysql_query("SELECT * FROM `brand` WHERE  `dept`='$dept' AND `sub_categoryname`='$subcategory' AND `status`=1");
					while($cat=mysql_fetch_array($ca)){ ?>
					<option value="<?php echo $cat['id'];?>" <?php if($cat['id'] == $brandname){echo "selected";}?>><?php echo $cat['brandname'];?></option>
...
?>									</select>
		...
		<div class="form-group">
			...
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
				<input required type="text" name="product_name" id="product_name" value="<?php echo $pname;?>" class="form-control" autocomplete="off" />
			</div>
			
			<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="price">Product Price <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
				<input required type="text" name="price" id="price" value="<?php echo $price;?>" class="form-control" autocomplete="off" />
			</div>
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="commission">Commission <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
				<input required type="text" name="commission" id="commission" value="<?php echo $commission;?>" class="form-control" autocomplete="off" />
			</div>
				<div class="form-group">
			<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="gst">GST (%)<span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
				<input required type="text" name="gst" id="gst" value="<?php echo $gst;?>" class="form-control" autocomplete="off" />
			</div>
		</div>
		</div>
		...
		<div class="form-group">
			<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="commission">Featured </label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
