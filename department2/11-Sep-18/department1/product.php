...
mysql_query("INSERT INTO `product` (`dept`,`category`,`subcategory`,`pname`,`date`,`dtime`,`commission`,`gst`,`image`,`price`,`brandname`,`department`) VALUES ('0','$catid[0]','$sub_categoryname','$product_name','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$commission','$gst','$img','$price','$brandname','$department')");
...
$catid = explode("@",$category);
$in=mysql_query("UPDATE `product` SET `category`='$catid[0]',`subcategory`='$sub_categoryname',`pname`='$product_name',`commission`='$commission',`gst`='$gst',image='$img',`price`='$price',`brandname`='$brandname' WHERE `id`='$id'");
if($in)
{
echo "<script type='text/javascript'>swal({
...
<div class="panel-body">
	...
	<form class="cmxform form-horizontal tasi-form" id="" method="post" name="product" enctype="multipart/form-data" >
		...
		<input type="hidden" name="department" id="department" value="Grocery" class="form-control" autocomplete="off" readonly />
			...
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12"><select name="category" id="category" class="form-control" required  onchange="subcategory(this.value)">
...
<select name="sub_categoryname" id="sub_categoryname" class="form-control selectpicker1" data-show-subtext="true" data-live-search="true" onchange="brand_fetch(this.value,<?php echo $dept1; ?>,category.value )" >
				<option value="">--- Select ---</option>
					<?php
					 $ca=mysql_query("SELECT * FROM `sub_category` WHERE `status`=1");
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Brand <!--<span class="red">*</span>--></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12" id="brandname1">
				...
				<?php	 
				   if($brandname != "")
					{
					 $ca=mysql_query("SELECT * FROM `brand` WHERE   `id`='$brandname' AND `status`=1");
					while($cat=mysql_fetch_array($ca)){ ?>
