...
if($num_brand < 1)
{
 ...
 $in = mysql_query("INSERT INTO `brand` (`dept`,`category`,`sub_categoryname`,`brandname`,`date`,`dtime`,`department`) VALUES ('$dept[0]','$category','$sub_categoryname','$brandname','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$dept[1]')");
if($in)
 {
...
extract($_POST);
 $dept = explode("@",$department);
$in=mysql_query("UPDATE `brand` SET `category`='$category',`sub_categoryname`='$sub_categoryname',`brandname`='$brandname',`dept`='$dept[0]',`department`='$dept[1]' WHERE `id`='$id'");
if($in){
echo "<script type='text/javascript'>swal({
