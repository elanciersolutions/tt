...
mysql_query("INSERT INTO `product` (`dept`,`category`,`subcategory`,`pname`,`date`,`dtime`,`commission`,`gst`,`image`,`price`,`brandname`,`department`) VALUES ('$dept[0]','$catid[0]','$sub_categoryname','$product_name','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$commission','$gst','$img','$price','$brandname','$dept[1]')");
...
$catid = explode("@",$category);
$dept = explode("@",$department);
$in=mysql_query("UPDATE `product` SET `category`='$catid[0]',`subcategory`='$sub_categoryname',`pname`='$product_name',`commission`='$commission',`gst`='$gst',image='$img',`price`='$price',`brandname`='$brandname',`dept`='$dept[0]',`department`='$dept[1]' WHERE `id`='$id'");
if($in){
echo "<script type='text/javascript'>swal({
...
<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="department">Department <span class="red">*</span></label>
			...
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
              ...
              <select name="category" id="category" class="form-control" required  onchange="subcategory(this.value)">
...
<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Sub Category <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                 <select required name="sub_categoryname" id="sub_categoryname" class="form-control selectpicker" data-show-subtext="true" data-live-search="true" onchange="brand_fetch(this.value,category.value,department.value)" >
				<option value="">--- Select ---</option>
					<?php
		...
		<label class="col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label" for="pname">Brand <span class="red">*</span></label>
			<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12" id="brandname1">
				...
				<select name="brandname" required id="brandname" class="form-control" >
				<option value="">--- Select ---</option>
				<?php	 
				   if($brandname != "")
					{
					
				   $ca=mysql_query("SELECT * FROM `brand` WHERE  `dept`='$dept' AND `id`='$brandname' AND `status`=1");
					while($cat=mysql_fetch_array($ca)){ ?>
