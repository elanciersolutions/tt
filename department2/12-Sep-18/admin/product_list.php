...
<section id="flip-scroll">
	<table class="display table table-bordered cf" id="dyntable">
		...
		<tbody>
			...
			<tr class="text-center" id="<?php echo $i;?>" onchange="product_success(<?php echo $result['id'];?>,'<?php echo $result['pname'];?>',<?php echo $i;?>);price_success(<?php echo $result['id'];?>,<?php echo $result['price'];?>,<?php echo $i;?>)">
				...
				?></td>
                 <td class="text-center" ><?php $subid = $result['subcategory'];
						...
						$subname = mysql_fetch_array(mysql_query("SELECT * FROM sub_category WHERE dept='".$result['dept']."' and `id` ='$subid'")); echo  $subname['sub_categoryname'];?></td>
                 <td class="text-center" ><?php $brandid = $result['brandname'];
						
						$subname = mysql_fetch_array(mysql_query("SELECT * FROM brand WHERE dept='".$result['dept']."' and `id` ='$brandid'")); echo  $subname['brandname'];?></td>
				...
				<?php } ?><a href="product.php?id=<?php echo $result['id'];?>" class="block_tip tooltips"><i class="fa fa-pencil"></i></a><a  onclick="del_product(<?php echo $result['id'];?>)" class="block_tip tooltips"><i class="fa fa-times"></i></a></td>
			</tr>
			<?php $i++; } ?>
		</tbody>
	</table>
