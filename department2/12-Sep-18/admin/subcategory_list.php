...
<table class="display table table-bordered cf" id="dyntable">
	...
	<tbody>
		...
		$sub = mysql_query("SELECT * FROM sub_category where status=1");
		
		$i=1; while($result=mysql_fetch_array($sub)) {?>		
		<tr>
			<td class="text-center"><?php echo $i;?></td>
			<td class="text-center"><?php echo $result['department'];?>
			</td>
			...
			<input type="checkbox" checked data-toggle="toggle" data-on="Ready" data-off="Not Ready" data-onstyle="success" data-offstyle="danger" onclick="app_status(1,<?php echo $result['id'];?>,<?php echo $result['dept'];?>)" id="disable">
			<?php } else if($result['appstatus']==0){?>
			<input type="checkbox"  data-toggle="toggle" data-on="Ready"  data-onstyle="success" data-off="Not Ready"  data-offstyle="danger" onclick="app_status(0,<?php echo $result['id'];?>,<?php echo $result['dept'];?>)" id="enable">
			<?php } ?></td>
