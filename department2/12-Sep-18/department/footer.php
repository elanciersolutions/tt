	...
	/*********** Find SubCategory  ***************/
		...
		function dynamic_purchase(pname,price,gst,commission,stock,tr_count,subname,brandname,category_name)
		{
			...
			var data = '<tr id="row'+tr_count+'"><td>'+tr_count+'</td><td>'+pname[1]+'<input type="hidden" name="pname[]" value="'+pname[1]+'"><input type="hidden" name="category_name[]" value="'+category_name+'"></td><td>'+price+'<input type="hidden" name="price[]" value="'+price+'"></td><td>'+gst+'%<input type="hidden" name="gst[]" value="'+gst+'"></td><td>'+commission+'<input type="hidden" name="commission[]" value="'+commission+'"></td><td>'+stock+'<input type="hidden" name="stock[]" value="'+stock+'"><input type="hidden" name="subname[]" value="'+subname+'"><input type="hidden" name="brandname[]" value="'+brandname+'"></td><td><a  class="block_tip tooltips text-center" onclick="remove_tr('+tr_count+')"><i class="fa fa-times"></i></a></td></tr>';
 ...
 function multiple_print(salesid,dept,userid) 
{
 ...
 function price_success(id,price,trid)
   {  
	...
	function subcategory_purchase(catid,dept)
	{
  
       $.ajax({
			    
                url     : 'subcategory_purchase.php',
                type    : 'POST',				
				data    : {'catid':catid,'dept':dept},
   ...
   /********* Purchase Brand ********/
    ...
    function brand_fetch_purchase(subid,dept,catid)
		{
			
			$.ajax({
			    
                url     : 'brand_fetch_purchase.php',
                type    : 'POST',				
				data    : {'subid':subid,'dept':dept,'catid':catid},
   ...
   /********* Add Supplier  *********/
    ...
    function add_supplier(q7,q6,q5,q4,q3,q2,q1)
    { 
       if(q1 != "")
		{
       ...
       $.ajax({
                ...
                type    : 'POST',				
				data    : {'q7':q7,'q6':q6,'q5':q5,'q4':q4,'q3':q3,'q2':q2,'q1':q1},
