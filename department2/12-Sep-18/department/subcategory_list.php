...
<table class="display table table-bordered cf" id="dyntable">
	...
	<tbody>
		...
		$sub = mysql_query("SELECT * FROM sub_category WHERE `dept`='$dept1' ORDER BY `id` DESC");
		...
		<tr>
			...
			<input type="checkbox" checked data-toggle="toggle" data-on="Ready" data-off="Not Ready" data-onstyle="success" data-offstyle="danger" onclick="app_status(1,<?php echo $result['id'];?>,<?php echo $dept1;?>)" id="disable">
			<?php } else if($result['appstatus']==0){?>
			<input type="checkbox"  data-toggle="toggle" data-on="Ready"  data-onstyle="success" data-off="Not Ready"  data-offstyle="danger" onclick="app_status(0,<?php echo $result['id'];?>,<?php echo $dept1;?>)" id="enable">
			<?php } ?></td>
