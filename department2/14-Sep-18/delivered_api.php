<?php  include("../admin/connect/config.php");
	   ...
	   $approve = mysql_query("select * from sales where `approve_status`=1 and `cscid`='$cscid' and approve_date='$today' order by `id` asc LIMIT $start_limit,$end_limit");
	   
	   $num = mysql_num_rows(mysql_query("select * from sales where approve_status=1 and cscid='$cscid' and approve_date='$today'"));
