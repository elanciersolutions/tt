...
if(isset($_POST['submit'])){
	...
	{
	 echo $exisit_product = mysql_num_rows(mysql_query("SELECT * FROM addpurchase WHERE `product_name`='$pname[$i]' AND `category_name`='$category' "));		
	 if($exisit_product < 1)
	 {
	...
	$in = mysql_query("INSERT INTO `temp_purchase` (`category_name`,`product_name`,`subcategory`,`price`,`commission`,`purchase_date`,`dtime`,`stock`,`dept`,`gst`,`brandname`) VALUES ('$category','$pname[$i]','$subname[$i]','$price[$i]','$commission[$i]','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','$stock[$i]','$dept','$gst[$i]','$brandname[$i]')");
		 ...
		 $product_stock = mysql_fetch_array(mysql_query("SELECT * FROM addpurchase WHERE `product_name`='$pname[$i]' AND `category_name`='$category' "));	
